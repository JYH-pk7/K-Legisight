...
MEETING_ID = 51007  # 회의 번호만 바꿔가며 사용
...
def is_trigger_candidate(text: str) -> bool:
    ...
    # 🚫 절대 트리거 후보가 될 수 없는 procedural 표현
    #    예: "정회하겠습니다", "산회를 선포합니다"
    if "정회" in text or "산회" in text:
        return False
...
1) 심사/설명/보고/재심사 “선언이 있는 문장만” 트리거로 취급한다.
...
2) “상정만 한 구간”과 “소위원회 계속심사만 언급한 구간”은 제외한다.
   ...
   - 같은 발언 안에서 뒤쪽에
     ...
     와 같이 **실제 심사/설명/보고를 시작하는 구간이 나오면**,
     → 그 뒤의 심사 구간만 트리거로 보고, 해당 숫자들만 agenda_items에 넣는다.
...
4) 한 발언 안에 여러 구간(상정/의결/계속심사/새 심사 시작 등)이 섞여 있을 경우,
   “가장 마지막에 등장하는 실제 심사/설명/보고 구간만” agenda_items로 사용한다.
   - 예)
     "의사일정 제1항부터 제47항까지 이상 47건의 법률안을 일괄하여 상정합니다.
...
예시 7) (여러 의결 후, 다음 심사 구간 시작, 345번 패턴)
발언:
"… 의사일정 제4항 … 가결되었음을 선포합니다. … 의사일정 제25항 … 가결되었음을 선포합니다.
의사일정 제32항 … 가결되었음을 선포합니다.
다음은 의사일정 제5항부터 제7항까지 이상 3건의 자본시장과 금융투자업에 관한 법률 일부개정법률안을 일괄하여 심사하도록 하겠습니다."
...
예시 12) (앞 구간을 제시하고, 이어지는 문장에서 추가 구간을 함께 심사한다고 하면 두 구간을 합친다)
...
예시 14) (오늘 방금 심사한 안건에 대해 계속심사만 선언하는 경우)
...
예시 15) (의사일정 여러 건을 일괄 상정하면서, 곧바로 수석전문위원 설명을 요청하는 경우 – 실제 심사 개시)
발언:
...
예시 17) (문장이 끊겼고 뒤에 ‘심사/설명/보고’가 아직 없는 경우는 트리거 아님)
발언: 
